...
{	int j = 0, h = now._nr_pr;
	...
	case 0:	/* p1 */
		...
		((P0 *)pptr(h))->n2 = 0;
#ifdef VAR_RANGES
		logval("p1:msg", ((P0 *)pptr(h))->msg);
		logval("p1:n1", ((P0 *)pptr(h))->n1);
		logval("p1:n2", ((P0 *)pptr(h))->n2);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.canal1 = addqueue(calling_pid, 1, 0);
		now.canal2 = addqueue(calling_pid, 2, 0);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.canal1) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.canal1) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.canal1) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.canal1) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.canal2) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.canal2) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.canal2) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.canal2) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan canal1 (=%d):	len %d:\t", now.canal1, q_len(now.canal1));
	c_chandump(now.canal1);
	printf("	chan canal2 (=%d):	len %d:\t", now.canal2, q_len(now.canal2));
	c_chandump(now.canal2);
}
void
